...
const Verify = () => {
  ...
  const sendVerificationMemberEmail = async () => {
    console.log("이메일 인증 버튼 누름");
    console.log("이메일: " + memberEmail);
    const response = await fetch(
      "http://localhost:10000/api/sms/email/verification-code",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  // 인증 번호를 토대로 이메일 인증 진행
  const verifyMemberEmail = async () => {
    const response = await fetch(
      "http://localhost:10000/api/sms/email/verification-code/verify",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      <div>
        ...
        {!isVerified.memberPhoneVerification &&
          (isSent ? (
            ...
            <button onClick={sendVerificationMemberPhone}>
              인증 번호 발송
            </button>
          ))}
      </div>
      <div>
        ...
        {!isVerified.memberEmailVerification &&
          (isEmailSent ? (
            ...
            <button onClick={sendVerificationMemberEmail}>
              인증 번호 발송
            </button>
          ))}
      </div>
